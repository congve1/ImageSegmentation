...
class VGG16:
    ...
    def convlayers(self):
        ...
        with tf.name_scope("conv1_2") as scope:
            ...
            conv = self.conv2d(self.conv1_1,kernel)
            biases = tf.Variable(tf.constant(0.0,dtype=tf.float32,shape=[64]),trainable=True,name="biases")
            out = tf.nn.bias_add(conv,biases)
        ...
        with tf.name_scope("conv2_1") as scope:
            ...
            conv = self.conv2d(self.pool1,kernel)
        ...
        with tf.name_scope("conv2_2") as scope:
            ...
            conv = self.conv2d(self.conv2_1,kernel)
        ...
        with tf.name_scope("conv3_1") as scope:
            ...
            conv = self.conv2d(self.pool2,kernel)
        ...
        with tf.name_scope("conv3_2") as scope:
            ...
            conv = self.conv2d(self.conv3_1,kernel)
            biases = tf.Variable(tf.constant(0.0,dtype=tf.float32,shape=[256]),trainable=True,name="biases")
            out = tf.nn.bias_add(conv,biases)
        ...
        with tf.name_scope("conv3_3") as scope:
            kernel = tf.Variable(tf.truncated_normal([3,3,256,256],dtype=tf.float32,stddev=1e-1),name="weights")
            conv = self.conv2d(self.conv3_2,kernel)
            biases = tf.Variable(tf.constant(0.0,dtype=tf.float32,shape=[256]),trainable=True,name="biases")
            out = tf.nn.bias_add(conv,biases)
        ...
        with tf.name_scope("conv4_1") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 256, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.pool3  , kernel)
        ...
        with tf.name_scope("conv4_2") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.conv4_1, kernel)
            biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[512]), trainable=True, name="biases")
            out = tf.nn.bias_add(conv, biases)
        ...
        with tf.name_scope("conv4_3") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.conv4_2, kernel)
            biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[512]), trainable=True, name="biases")
            out = tf.nn.bias_add(conv, biases)
        ...
        with tf.name_scope("conv5_1") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.pool4  , kernel)
            biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[512]), trainable=True, name="biases")
            out = tf.nn.bias_add(conv, biases)
        ...
        with tf.name_scope("conv5_2") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.conv5_1, kernel)
            biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[512]), trainable=True, name="biases")
            out = tf.nn.bias_add(conv, biases)
        ...
        with tf.name_scope("conv5_3") as scope:
            kernel = tf.Variable(tf.truncated_normal([3, 3, 512, 512], dtype=tf.float32, stddev=1e-1),
                                 name="weights")
            conv = self.conv2d(self.conv5_2, kernel)
            biases = tf.Variable(tf.constant(0.0, dtype=tf.float32, shape=[512]), trainable=True, name="biases")
            out = tf.nn.bias_add(conv, biases)
    ...
    def fc_layers(self):
        ...
        with tf.name_scope("fc1") as scope:
            ...
            fc1b = tf.Variable(tf.constant(1.0,dtype=tf.float32,shape=[4096]),trainable=True,name='biases')
        ...
        with tf.name_scope("fc2") as scope:
            ...
            fc2b = tf.Variable(tf.constant(1.0,dtype=tf.float32,shape=[4096]),trainable=True,name='biases')
        ...
        with tf.name_scope("fc3") as scope:
            ...
            fc3b = tf.Variable(tf.constant(1.0,dtype=tf.float32,shape=[1000]),trainable=True,name='biases')
...
vgg = VGG16(imgs,"/home/clw/PycharmProjects/VGG16/vgg16_weights.npz",sess)
def inference(vgg,filename):
    img1 = imread(filename,mode="RGB")
    img1 = imresize(img1,(224,224))
    prob = sess.run(vgg.probs,feed_dict={imgs:[img1]})[0]
    preds = (np.argsort(prob)[::-1])[0:5]
    res_names = []
    res_probs = []
    for idx in preds.astype(np.int32):
        res_names.append(class_names[idx])
        res_probs.append(prob[idx])
    return res_names,res_probs
...
if __name__ == "__main__":
    ...
    preds = (np.argsort(prob)[::-1])[0:5]
    for p in preds:
        print(class_names[int(p)],prob[int(p)])
